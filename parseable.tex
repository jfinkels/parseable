...
First we make the following conjecture about the general recognition (that is, parsing) problem for the class of permuted indexed languages.
\begin{conjecture}
...
\end{lemma}
\begin{proof}
  The witness is the permutation, whose length is polynomial with respect to the set of input symbols.
  The verification procedure is the procedure for parsing a string given a context-free grammar, which is computable in polynomial time.
\end{proof}

\begin{openproblem}
  Is \lang{CLOSED} a mildly context-sensitive language? Could it be decided by, for example, an instance of the formalism described in \cite{kallmeyer}? If so, then it is decidable in polynomial time, and hence $\lang{\pc}\in\NP$.
\end{openproblem}
