...
\begin{definition}
  ...
  $L$ has the \emph{constant growth property} if there is a contstant $c_0>0$ and a finite set of positive constants $C\subseteq\mathbb{N}$ such that for all $w\in L$ with $|w|>c_0$, there exists a $w'\in L$ with $|w|=|w'|+c$ for some $c\in C$.
\end{definition}
\begin{definition}\mbox{}
  \begin{itemize}
